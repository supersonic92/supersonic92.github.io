...
const renderTrendings = async () => {
  try {
    ...
    if (trendings) {
      ...
      trendings.data.map((x) =>
        CreateCard(
          x,
          ["trending"],
          containerCarrousel,
          (x) => {
            x.parentElement.children[0].onclick = (e) =>
              AbrirModal(
                e.target.parentElement.children[1].children[0].children[2]
                  .children[0]
              );
            x.children[0].children[2].onclick = (e) => AbrirModal(e.target);
            x.children[0].children[1].onclick = (e) =>
              downloadGif(
