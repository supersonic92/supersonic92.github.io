let AbrirModal = (e) => {
  let target =
    e.parentElement.parentElement.parentElement.parentElement.children[0];
  containerModal.children[3].children[0].value =
    e.parentElement.parentElement.parentElement.children[2].children[0].value;
...
const DowloadModalImg = (e) => {
  let CriterioDescarga =
    e.target.parentElement.parentElement.parentElement.parentElement.children[3]
      .children[0].value;
  let imagen =
...
const ModalFavoritos = (e, criterioDescargas) => {
  ...
  let Auxiliar = [];
  localStorageGifs
    ? JSON.parse(localStorageGifs).map((e) => Auxiliar.push(e))
    : [];
  let verificacion = Auxiliar.filter(
    (x) => x.title == target.getAttribute("data-title")
  );
  ...
  } else console.log("Ya existe en favoritos");
};
