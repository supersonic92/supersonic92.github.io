...
let gifoUrl =
  "https://api.giphy.com/v1/gifs/trending?api_key=wUIs2kykDiUjqc9ljNRoH97ddpN05IwD&limit=12&offset=5&rating=g&random_id=e826c9fc5c929e0d6c6d423841a282aa";
...
let downloadGif = (objetoADescargar, CriterioDeBusqueda) => {
  ...
  CriterioDeBusqueda.map(async (item) => {
    if (item.title == TitleImagen) {
      await fetch(urlImagen).then((img) => {
        img.blob().then((file) => {
